...
test.renderHead = function (isBrowser) {
  ...
  if (isBrowser) {
    ...
    // Reset Filters
    test.tequilaStats = '☠';
    test.helpTestTequila = 'game interface ' + T.getVersion() + '<br>click filters for test pass/fail and selected session<b><em>note: click on any section below to filter</em></b>';
    test.btnTequila = buttonControl(test.tequilaStats + ' game interface',
...
test.updateStats = function () {
  ...
  if (test.tequilaStats != test.countUnique + newtequilaStats) {
    ...
    test.btnTequila.innerHTML = 'tests<br><code class="counter">' + miniPad + myName + '</code>' + '<span class="classic">' + test.helpTestTequila + '</span>';
  }
